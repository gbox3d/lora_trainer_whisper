...
- `src/` 패키지 구조 전환 완료 (2026-04-09). `dataset_tools` + `lora_trainer` 두 패키지로 분리, dalus_server 호환 래퍼 유지.
- `dalus_server` Whisper 브랜치가 이 저장소를 안정적으로 호출할 수 있도록, CLI/산출물 계약을 유지한다.
...
4. (완료) `src/` 패키지 구조 전환 + `dev_dataset_tools`의 validate 기능 통합
