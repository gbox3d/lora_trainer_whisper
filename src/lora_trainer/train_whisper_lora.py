...
class StepTimingCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        self._last_log_time = self._train_start
        print(f"[timing] Training started")

    def on_log(self, args, state: TrainerState, control: TrainerControl, logs: dict, **kwargs):
        now = time.monotonic()
        if self._last_log_time is not None:
            delta = now - self._last_log_time
            elapsed = now - (self._train_start or now)
            print(
                f"[timing] step={state.global_step}"
                f"  +{delta:.1f}s since last log"
    ...
    def on_epoch_end(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        now = time.monotonic()
        elapsed = now - (self._train_start or now)
        print(f"[timing] epoch={state.epoch:.0f} ended  total={elapsed/60:.1f}min")
